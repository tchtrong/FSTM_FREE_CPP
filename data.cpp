...
{ //this will read a file in either Libsvm format or LDA format
	...
	double count;
	char *token, *str = (char *)std::malloc(100000);
	const char delimiters[] = " :";

	printf("reading data from %s\n", data_filename);
	c = reinterpret_cast<corpus *>(std::malloc(sizeof(corpus)));
	c->label_names = reinterpret_cast<int *>(std::malloc(sizeof(int)));
	c->docs = 0;
	c->num_terms = 0;
	c->num_docs = 0;
	c->num_labels = 0;
	...
	{
		OFFSET = 0;
		printf("unsupervised data.\n");
	}
	else
	{
		OFFSET = 1;
		printf("multi-class data.\n");
	}
	nd = 0;
	nw = 0;
	while (1)
	{
		...
		c->docs[nd].total = 0;
		c->docs[nd].entropy = 0;
		c->docs[nd].words = reinterpret_cast<int *>(std::malloc(sizeof(int) * length));
		c->docs[nd].counts = reinterpret_cast<double *>(std::malloc(sizeof(double) * length));
...
void free_corpus(corpus *corp)
{
	...
	{
		std::free(corp->docs[n].words);
		std::free(corp->docs[n].counts);
	}
	if (corp->labels != NULL)
		for (n = corp->num_docs - 1; n > 0; n--)
			std::free(corp->labels[n]);
	if (corp->label_names != NULL)
		std::free(corp->label_names);
	std::free(corp->docs);
	std::free(corp);
}
...
corpus *new_corpus(int num_docs, int num_terms)
{
	...
	c->num_docs = num_docs;
	c->docs = (document *)std::malloc(sizeof(document) * num_docs);
	if (c->docs == NULL)
	{
	...
	{
		c->docs[i].words = (int *)std::malloc(sizeof(int));
		c->docs[i].counts = (double *)std::malloc(sizeof(double));
		c->docs[i].length = 0;
	}
	return (c);
}
