...
void fstm_Learn(char *directory, corpus *corp)
{
	...
	b_ind = (int *)std::malloc(sizeof(int) * corp->num_terms);
	for (d = 0; d < corp->num_terms; d++)
		b_ind[d] = 1;
	...
	while (((converged > EM_CONVERGED) || (i <= 2)) && (i <= EM_MAX_ITER))
	{
		...
		update_sparse_topics(model, corp, aa, i, b_ind);
...
{	//project a document to the simplex of topics, using Frank-Wolfe algorithm [Clarkson10]
	...
	opt = (double *)std::malloc(sizeof(double) * doc.length);
	x = (double *)std::malloc(sizeof(double) * doc.length);
	sum = (double *)std::malloc(sizeof(double) * model->num_topics);
	temp = (double *)std::malloc(sizeof(double) * model->num_topics);
		...
		for (j = 0; j < doc.length; j++)
			if (model->bb[doc.words[j]].length > 0)
				for (k = 0; k < model->bb[doc.words[j]].length; k++)
					...
					{
						opt[j] = ep * model->bb[doc.words[j]].value[k] + EPS;
						break;
					}
		for (k = 0; k < model->num_topics; k++)
			temp[k] = 0;
		...
		for (j = 0; j < doc.length; j++)
		{
			...
			if (model->bb[doc.words[j]].length > 0)
				for (k = 0; k < model->bb[doc.words[j]].length; k++)
					if (ind == model->bb[doc.words[j]].topicID[k])
					{
						x[j] += ep * model->bb[doc.words[j]].value[k] + EPS;
						break;
					}
		}
		//Search for alpha
	...
	if (j == 0)
		j = 1;
	aa->docs[docID].words = (int *)std::malloc(sizeof(int) * j);
	aa->docs[docID].counts = (double *)std::malloc(sizeof(double) * j);
	...
	for (k = 0; k < model->num_topics; k++)
		if (temp[k] > 0)
		{
			aa->docs[docID].words[j] = k;
			aa->docs[docID].counts[j] = temp[k];
			j++;
		}
	std::free(opt);
	std::free(sum);
...
void warm_start_init(fstm_model *model, document doc, document coeff, double *opt, double *temp, double ep)
{
			...
			for (t = 0; t < coeff.length; t++)
				if (coeff.counts[t] > 0)
					for (k = 0; k < model->bb[doc.words[j]].length; k++)
						...
						{
							opt[j] += coeff.counts[t] * model->bb[doc.words[j]].value[k];
							break;
						}
	for (j = 0; j < doc.length; j++)
		opt[j] = ep * opt[j] + EPS;
	for (k = 0; k < model->num_topics; k++)
		temp[k] = 0;
	for (t = 0; t < coeff.length; t++)
		temp[coeff.words[t]] = coeff.counts[t];
}
...
{ //update the topics when all are in sparse format
	...
	for (d = 0; d < corp->num_docs; d++)
		for (j = 0; j < corp->docs[d].length; j++)
			for (k = 0; k < aa->docs[d].length; k++)
				temp[aa->docs[d].words[k]] += aa->docs[d].counts[k] * corp->docs[d].counts[j];
	for (k = 0; k < model->num_topics; k++)
		temp[k] = 1.0 / temp[k];
	...
	for (d = 0; d < corp->num_docs; d++)
		for (j = 0; j < corp->docs[d].length; j++)
		{
			...
			{
				tp = -1;
				if (b_ind[wid] > EM_iter) //dimension 'wid' has been initialized
					for (i = 0; i < model->bb[wid].length; i++)
						if (model->bb[wid].topicID[i] == aa->docs[d].words[k])
						{
							tp = i;
							break;
						}
				if (tp >= 0)
					model->bb[wid].value[tp] += aa->docs[d].counts[k] * counts * temp[aa->docs[d].words[k]];
				else
				{ //new item
					//if this dimension has been initialized?
					if (b_ind[wid] <= EM_iter)
					{
						model->bb[wid].length = 0;
						b_ind[wid] = EM_iter + 1;
					}
					model->bb[wid].length++;
					model->bb[wid].topicID[model->bb[wid].length - 1] = aa->docs[d].words[k];
					model->bb[wid].value[model->bb[wid].length - 1] = aa->docs[d].counts[k] * counts * temp[aa->docs[d].words[k]];
				}
			}
		}
	std::free(temp);
...
{
	int i, j;
	double fl, fr, fa, alpha, left, right;

	fl = 0;
	fr = 0;
	for (i = 0; i < doc.length; i++)
	{
		fr += log(opt[i] + x[i]) * doc.counts[i];
		fl += log(opt[i]) * doc.counts[i];
	}
	left = 0;
	right = 1;
	for (j = 0; j < 20; j++)
	{
		...
		fa = 0;
		for (i = 0; i < doc.length; i++)
			fa += log(alpha * x[i] + opt[i]) * doc.counts[i];
		if (fa >= fr)
		{
			fr = fa;
			right = alpha;
...
{ //binary search with direction of Gradient
	...
	for (j = 0; j < 20; j++)
	{
		...
		for (i = 0; i < doc.length; i++)
			if (x[i] != 0)
				fa += doc.counts[i] * x[i] / (alpha * x[i] + opt[i]);
		if (abs(fa) < 1e-10)
			break;
		if (fa < 0)
			right = alpha;
