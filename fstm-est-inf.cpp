...
{
	int d, i, LAG, *b_ind;	fstm_model *model;		corpus *aa;
	...
	b_ind = (int*) malloc(sizeof(int)*corp->num_terms); 
	for (d = 0; d < corp->num_terms; d++) b_ind[d] = 1;
...
{	//project a document to the simplex of topics, using Frank-Wolfe algorithm [Clarkson10]
    ...
    int k, j, ind, t;
	if (docID % 1000 == 0) {printf("%d, ", docID); fflush(stdout);}
...
{//update the topics when all are in sparse format
	...
	for (k = 0; k < model->num_topics; k++) temp[k] = 1.0/temp[k];
	...
	for (d = 0; d < corp->num_docs; d++) 
		for (j = 0; j < corp->docs[d].length; j++)
		{
			...
			{	tp = -1;
				...
				if (tp >= 0) model->bb[wid].value[tp] += aa->docs[d].counts[k] * counts * temp[aa->docs[d].words[k]];
				else{ //new item
					//if this dimension has been initialized?
					if (b_ind[wid] <= EM_iter) 	{ model->bb[wid].length = 0;  b_ind[wid] = EM_iter +1; }	
					model->bb[wid].length ++;
					model->bb[wid].topicID[model->bb[wid].length -1]	= aa->docs[d].words[k];
					model->bb[wid].value[model->bb[wid].length -1]		
						= aa->docs[d].counts[k] * counts * temp[aa->docs[d].words[k]];
				}
			}
		}
	free(temp);
